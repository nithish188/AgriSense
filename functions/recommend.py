...
CROP_TRANSLATIONS = {
    'Rice':      'நெல் (Nel)', 'Maize': 'மக்கா சோளம் (Makka Cholam)',
    'Cotton':    'பருத்தி (Paruthi)', 'Soybean': 'சோயா பீன்ஸ் (Soya Beans)',
    'Wheat':     'கோதுமை (Godhumai)', 'Sugarcane': 'கரும்பு (Karumbu)',
    'Coffee':    'காபி (Kapi)', 'Apple': 'ஆப்பிள் (Apple)',
    'Mango':     'மாம்பழம் (Mambalam)', 'Grapes': 'திராட்சை (Dhiratchai)'
}
...
def handler(event, context):
    ...
    try:
        ...
        if not city:
            return {"statusCode": 400, "body": json.dumps({"error": "City name is required."})}
        if soil_type not in SOIL_PROPERTIES:
            return {"statusCode": 400, "body": json.dumps({"error": f"Invalid soil type: {soil_type}"})}
        ...
        try:
            ...
            if not geo_resp.get('results'):
                return {"statusCode": 404, "body": json.dumps({"error": f'City "{city}" not found.'})}
            ...
            city_name, temp, humidity, precipitation = city, 28.0, 70.0, 0.0
        ...
        result = {
            ...
            'soil': {'type': soil_type, 'N': N, 'P': P, 'K': K, 'ph': ph},
            'weather': {
                'city': city_name,
